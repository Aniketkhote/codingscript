		        	...
		        	<ul class="discussions">
		        		...
		        		@foreach($discussions as $discussion)
							<li>
				        		<a class="discussion_list" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		...
					        		<div class="chatter_middle">
										<span class="chatter_middle_details" data-href="/user">
											<h5>
												{{ ucfirst($discussion->user->{Config::get('chatter.user.database_field_with_user_name')}) }}
											</h5>
											<span class="posted_at">Posted at : {{ \Carbon\Carbon::createFromTimeStamp(strtotime($discussion->created_at))->diffForHumans() }}</span>
										</span>
										<hr>
							...
							<div class="no_post col-md-6 offset-2 mt-5">
								<h3 class="text-uppercase">Oops! Not Yet Any Discussion</h3>
								<p class="text-muted">Sorry but the discussion you are looking are not available.</p>
							</div>
							@endif
		        	</ul>
	        	</div>
